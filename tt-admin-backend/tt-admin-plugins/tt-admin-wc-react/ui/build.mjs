...
function scanModuleEntries(allowedModules = []) {
  ...
  if (fs.existsSync(modulesDir)) {
    fs.readdirSync(modulesDir).forEach(moduleName => {
      ...
      const entry = candidates.map(candidate => path.join(moduleDir, candidate)).find(candidate => fs.existsSync(candidate));
...
async function buildModules() {
  ...
  const pluginId = pluginSetting.plugin?.id || 'tt-plugin-wc-react';
