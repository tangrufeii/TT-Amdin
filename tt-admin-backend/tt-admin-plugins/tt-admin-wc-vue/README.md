...
Web-Component Vue 模板插件，支持宿主 `plugin-dev` 热更新。
...
- 编辑 `ui/src/modules/wc-vue/view/WcVueView.vue`
- 页面会通过 `__TT_PLUGIN_HMR_RELOAD__` 自动刷新当前插件视图

## 可选：独立调试插件前端

- `pnpm -C tt-admin-backend/tt-admin-plugins/tt-admin-wc-vue/ui dev`

## 打包

- `mvn -pl tt-admin-wc-vue -am package -DskipTests`
