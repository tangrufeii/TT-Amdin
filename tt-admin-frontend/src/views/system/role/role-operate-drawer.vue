...
async function handleSubmit() {
  await validate();
  const func = isEdit.value ? fetchUpdateRole : fetchAddRole;
  const { error, data } = await func(model as Api.SystemManage.RoleEdit & { id: number });
  if (!error && data) {
    ...
    <NDrawerContent :title="title" :native-scrollbar="false" closable>
      <NForm ref="formRef" :model="model" :rules="rules">
        ...
        <NFormItem :label="$t('page.manage.role.sort')" path="sort">
          <NInputNumber v-model:value="model.sort" class="w-full" :min="0" />
        </NFormItem>
        <NFormItem :label="$t('page.manage.role.descriptionField')" path="description">
