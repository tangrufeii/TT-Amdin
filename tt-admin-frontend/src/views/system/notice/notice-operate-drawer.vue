...
import { fetchAddNotice, fetchUpdateNotice } from '@/service/api';
...
type Model = Api.SystemManage.NoticeEdit;
...
function handleInitModel() {
  ...
  if (props.operateType === 'edit') {
    Object.assign(model, props.rowData, {
      releaseTime: props.rowData.releaseTime ? Date.parse(props.rowData.releaseTime) : Date.now()
    });
  }
}
...
async function handleSubmit() {
  await validate();
  const func = isAdd.value ? fetchAddNotice : fetchUpdateNotice;
  const { error, data } = await func(model as Api.SystemManage.NoticeEdit & { id: number });
  if (!error && data) {
